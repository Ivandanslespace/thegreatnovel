#!/usr/bin/env python3
"""Simple test for P0-4 genre assumptions fix."""

import sys
from tools.create_save import answers_to_package, normalize_package

# Test 1: Ship no disaster world
ship_world = {
    "name": "星舰家园",
    "theme": "星际旅行",
    "difficulty": "休闲",
    ...
    "world_blueprint": {
        ...
        "recipes": [{"id": "synthesize_food", "name": "食物合成", "materials": {"能源": 50}}],
        ...
        "starting_inventory": {
            ...
            "equipment": [{"id": "nav_device", "durability": 10}],
...
try:
    print("测试 1: 无灾难星舰世界...")
    supplied_world, supplied_talent = answers_to_package(ship_world)
    template = {"world": {}, "player_talent": {}}
    
    # Debug: check resources before normalize
    print(f"  Resources after answers_to_package: {supplied_world.get('resources')}")
    print(f"  Primary count: {len(supplied_world.get('resources', {}).get('primary', []))}")
    
    world, talent = normalize_package(template, supplied_world, supplied_talent)
    print(f"✓ 世界创建成功：{world['name']}")
    print(f"  Genre ID: {world.get('genre_contract', {}).get('id')}")
    print(f"  载具基地：{world['setting'].get('vehicle_base')}")
    print(f"  灾难周期：{world['setting'].get('disaster_cycle')!r}")
    print()
...
solo_world = {
    ...
    "resources": {
        "primary": [{"name": "淡水"}, {"name": "食物"}, {"name": "木材"}, {"name": "石材"}, {"name": "燃料"}],
    },
    "rules": {
        "death": {"type": "checkpoint"},
    },
...
try:
    ...
    supplied_world2, supplied_talent2 = answers_to_package(solo_world)
    template = {"world": {}, "player_talent": {}}
    world2, talent2 = normalize_package(template, supplied_world2, supplied_talent2)
    print(f"✓ 世界创建成功：{world2['name']}")
    print(f"  Genre ID: {world2.get('genre_contract', {}).get('id')}")
    print(f"  安全基地：{world2['setting'].get('safe_base')}")
    print(f"  灾难周期：{world2['setting'].get('disaster_cycle')!r}")
    print()
except Exception as e:
    print(f"✗ 测试失败：{e}")
    import traceback
...
print("- 支持非生存类世界（如星舰）创建")
print("- 支持稳定环境（无周期性灾难）")
print("- Genre contract 明确声明 requirements")
print("=" * 60)
