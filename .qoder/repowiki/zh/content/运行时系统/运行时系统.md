...
- [engine_runtime/unified_validator.py](file://engine_runtime/unified_validator.py)
- [engine_runtime/validators_aggregate.py](file://engine_runtime/validators_aggregate.py)
...
## 更新摘要
**所做更改**   
- 新增统一验证框架章节，详细介绍 unified_validator.py 和 validators_aggregate.py
- 更新世界编译器部分，增加集中式模式验证功能
- 在架构总览中添加验证流程
- 扩展依赖关系分析以包含新的验证组件
- 更新故障排查指南以涵盖验证相关问题
...
## 项目结构
运行时模块位于 engine_runtime，围绕运行时主控制器、状态机、事件总线、持久化、计算与评级、审计与呈现等子模块组织。**新增的统一验证框架**提供了集中的模式验证能力。模板与工具分别用于存档结构与命令行集成。
...
UV["unified_validator.py<br/>统一验证器"]
VA["validators_aggregate.py<br/>验证聚合器"]
end
subgraph "支撑能力"
...
- **新增** 统一验证框架：提供集中式的模式验证能力，确保世界配置的完整性和正确性。
...
## 架构总览
运行时采用"控制流+事件流+验证流"的三轨架构：
- 控制流由运行时主控制器驱动，按回合或动作推进；
- 事件流通过事件队列解耦业务逻辑，确保可追溯与可回放；
- **新增** 验证流在编译阶段对世界配置进行集中式模式验证；
- 世界编译阶段将模板转化为运行时对象，并通过验证框架确保配置有效性；
...
participant UV as "统一验证器(unified_validator)"
participant VA as "验证聚合器(validators_aggregate)"
...
### 统一验证框架（unified_validator, validators_aggregate）
- 职责
  - **新增** 提供集中式的模式验证能力
  - 整合多种验证规则和执行策略
  - 生成详细的验证报告和错误信息
- 核心组件
  - UnifiedValidator：统一验证入口，协调各个验证器
  - ValidatorsAggregate：验证器聚合器，批量执行和管理验证规则
- 验证流程
  - 配置加载 -> 规则解析 -> 批量验证 -> 结果聚合 -> 报告生成
- 优势
  - 集中管理所有验证逻辑
  - 统一的错误处理和报告格式
  - 可扩展的验证规则注册机制
...
```mermaid
flowchart TD
A["世界配置"] --> B["UnifiedValidator"]
B --> C["ValidatorsAggregate"]
C --> D["SchemaValidator"]
C --> E["ConstraintValidator"]
C --> F["RelationshipValidator"]
D --> G["验证结果"]
E --> G
F --> G
G --> H["验证报告"]
...
### 架构设计
统一验证框架采用分层架构设计，通过集中式管理确保世界配置的一致性和完整性：
...
class ValidatorsAggregate {
+execute_validations(config) ValidationResult[]
+aggregate_results(results) ValidationResult
+handle_errors(errors) ErrorSummary
}
class SchemaValidator {
+validate_schema(config) bool
+check_required_fields(config) bool
+validate_data_types(config) bool
}
class ConstraintValidator {
+validate_constraints(config) bool
+check_business_rules(config) bool
+validate_ranges(config) bool
}
class RelationshipValidator {
+validate_relationships(config) bool
+check_dependencies(config) bool
+validate_references(config) bool
}
UnifiedValidator --> ValidatorsAggregate
ValidatorsAggregate --> SchemaValidator
ValidatorsAggregate --> ConstraintValidator
ValidatorsAggregate --> RelationshipValidator
```
...
### 核心功能特性
- **集中式管理**：所有验证逻辑统一管理，避免分散在各模块中
- **模块化设计**：不同类型的验证逻辑独立实现，便于扩展和维护
- **统一报告**：生成标准化的验证报告，包含详细的错误信息和位置
- **批量执行**：支持并行执行多个验证规则，提高验证效率
- **错误聚合**：收集所有验证错误，一次性返回给用户

### 验证流程详解
1. **配置加载阶段**：解析世界配置文件和模板
2. **验证器注册**：动态注册各种类型的验证器
3. **批量验证执行**：并行执行所有注册的验证规则
4. **结果聚合**：汇总所有验证结果和错误信息
5. **报告生成**：生成结构化的验证报告
...
## 依赖关系分析
运行时对状态、事件、持久化、呈现、计算与审计存在强耦合；世界编译器依赖模板与契约；**新增的统一验证框架**为世界编译器提供集中式验证能力；工具层通过运行时 API 驱动。
...
RT --> UV["unified_validator"]
ST --> MD["models"]
WC --> WT["world_template.yaml"]
WC --> HC["host_contract.json"]
WC --> UV
UV --> VA["validators_aggregate"]
VA --> SV["schema_validator"]
VA --> CV["constraint_validator"]
VA --> RV["relationship_validator"]
...
- 呈现延迟：按需渲染与分页输出
- 资源回收：及时释放文件句柄与内存占用
- **新增** 验证优化：并行执行验证规则，缓存验证结果，避免重复验证

## 故障排查指南
- 常见问题
  - 世界编译失败：检查模板语法与契约约束
  - **新增** 验证失败：检查统一验证框架的错误报告，确认配置是否符合所有验证规则
  - 事件无效：核对事件载荷与状态前置条件
  - 持久化异常：确认路径权限与模板一致性
  - 回放不一致：检查事件顺序与幂等性
- 诊断手段
...
## 结论
运行时系统以事件驱动为核心，结合世界编译与投影机制，提供了可扩展、可审计、可回放的叙事引擎基础。**新增的统一验证框架**进一步增强了系统的健壮性和可维护性，通过集中式的模式验证确保世界配置的正确性和一致性。通过清晰的模块划分与契约约束，既保证了稳定性，也为二次开发与集成预留了充足空间。建议在生产环境中配合完善的审计、监控与备份策略，以获得最佳的可观测性与可靠性。
...
### 统一验证框架使用指南
- 验证器注册：通过统一验证框架注册自定义验证逻辑
- 配置验证：在世界编译前执行集中式验证
- 错误处理：获取详细的验证错误报告和修复建议
- 性能优化：利用并行验证和结果缓存提升性能
