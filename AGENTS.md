...
`templates/world_template.yaml`，由 LLM 一次性创作完整世界包；Python 只负责校验、登记为
注册表并结算已登记合同，不能按主题补地点、敌人、NPC、模块、配方、行动成本或事件。

### 模板零默认值原则（P0-3 修复）

**核心哲学**: 模板中的 `rules.*` 字段现在全为 null，LLM 必须显式定义每个要启用的机制。  
**禁止行为**: Python 不再从模板注入任何实际 gameplay 参数（如"每天一次探索"、"7 天灾难周期"、"70%掉落率"）。  

- 所有生存规则（基地保护、探索限制、死亡惩罚、灾难周期、PvP、等级系统）必须在 `rules.*` 中明确定义才生效。
- 使用 `mechanics.capabilities` 声明世界支持的能力系统，`mechanics.rulesets` 存放自定义规则集。
- 这避免了深合并注入的假通用规则，使每个世界真正由 LLM 原创。

- 职业、首日地点、敌人、NPC、模块、配方、资源、灾难、后续事件池和主角天赋均由本次世界包原创；不存在主题关键词对应的职业/能力列表，也没有通用 30 血敌人或 30 分钟探索兜底。
