...
1. 世界主题是什么？（例：永夜冰川 / 巨兽背部 / 废土列车 / 深渊裂隙）
2. 玩家的安全基地是什么？（例：不断行驶的雪地列车 / 移动巨兽背上的营地）
3. 外部主要危险是什么？（例：冰尸、极寒、燃料不足）
4. 最重要的基础资源有哪些？（3-5种，例：煤炭、食物、寒晶）
5. 主角的特殊天赋是什么？（例：可以预览下一站的三种灾难）
6. 玩家通过什么方式探索？（例：列车停车时下车搜索）
7. 每隔多久发生一次大型灾难？（例：每七天一次白夜风暴）
8. 你希望游戏有多残酷？（休闲 / 标准 / 硬核永久死亡）
9. 每段叙述的长度偏好？（1-10，1=极短~100字 / 5=中等~500字 / 7=标准~800字 / 10=极长~1500字）
10. 游戏语言？（任何语言均可，不填则默认中文）
```
