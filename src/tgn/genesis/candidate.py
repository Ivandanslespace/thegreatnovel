...
class CandidateWorldDraft:
    ...
    def __init__(
        ...
        computed_candidate_hash = self._compute_candidate_hash()
        if world_semantic_candidate_hash is not None and world_semantic_candidate_hash != computed_candidate_hash:
        ...
        object.__setattr__(self, "world_semantic_candidate_hash", computed_candidate_hash)

    def _compute_candidate_hash(self) -> str:
...
def compile_candidate_artifacts(
    ...
    blocked = not report.requirements_gate_passed or not assessment.binding_gate_passed
    status = "BLOCKED_REQUIREMENTS" if blocked else "READY_FOR_FUTURE_PREFLIGHT"
