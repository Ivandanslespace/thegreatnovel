...
def prepare_draft_task(
    ...
    with database.connect() as connection:
        row = connection.execute(
            "SELECT * FROM chapter_contracts WHERE book_id=? AND contract_id=?",
            (book_id, contract_id),
        ).fetchone()
        if row is None:
        ...
        revision = int(
            connection.execute(
                "SELECT COUNT(*) FROM drafts WHERE book_id=? AND contract_id=?",
                (book_id, contract_id),
            ).fetchone()[0]
        ) + 1
    if revision > 3:
...
def import_draft_output(
    ...
    draft_id = stable_id(
        "draft", output.contract_id, str(metadata["revision"]), content_hash
    )
    ...
    with database.connect() as connection:
        connection.execute(
            ...
            SELECT ?, ?, ?, candidate_id, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
            FROM chapter_contracts WHERE contract_id=? AND book_id=?
            """,
            (
                draft_id,
                book_id,
...
def show_draft(database: Database, book_id: str, draft_id: str) -> dict[str, object]:
    with database.connect() as connection:
        row = connection.execute(
            "SELECT * FROM drafts WHERE book_id=? AND draft_id=?", (book_id, draft_id)
        ).fetchone()
...
def discard_draft(database: Database, book_id: str, draft_id: str) -> dict[str, object]:
    with database.connect() as connection:
        row = connection.execute(
            "SELECT status FROM drafts WHERE book_id=? AND draft_id=?", (book_id, draft_id)
        ).fetchone()
        if row is None:
            raise DraftWorkflowError(f"草稿不存在：{draft_id}")
        if row["status"] in {
