# Phase 5 Semantic Fixture A/B Benchmark (Historical)

> `SEMANTIC_FIXTURE_AB / NOT_LIVE_GENERATION_BENCHMARK`

本文件是 Phase 5 的历史确定性 fixture 记录，不是真实 Live Codex handoff 结果。真正的两阶段实现见 `scripts/phase5_live_ab.py` 与 `benchmark/phase5_1_live_generation_ab.md`。

本报告记录历史 fixture 对 Distill-only（A）与 Distill + Runtime Fused（B）边界的确定性对照。它保留独立 Book、可见边界、Runtime 隔离和 hidden truth 延后揭示的测试形状，但其中的九维、候选和正文不是由本轮 READY_FOR_CODEX handoff 现场生成。
