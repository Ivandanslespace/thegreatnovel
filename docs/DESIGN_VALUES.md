...
边界，但不提前定义完整 Relationship Engine、关系图数据库或具体 WorldPack
内容。

## 49.1 引擎对性别组合保持中立

未来的关系规则必须能够以同一套引擎合同支持男男、男女、女女等组合，也不能
把当前项目永远限制为这三种身份表达。性别身份可以影响人物设定、私人偏好和
叙事表达，但不能直接决定引擎是否允许某种关系。

明确地说，`male-male`、`male-female` 和 `female-female` 使用同一套关系与
联合行动合同；未来更多身份组合也应沿用同一因果结构，而不是新增平行系统。
...
引擎不得为不同性别组合建立三套并行系统，也不得把下面这些题材称呼变成
通用字段：
...
这些词可以出现在特定 WorldPack 或 Narrator 文本中，但不应成为 Core 的关系
模型。不得出现根据玩家和 NPC 性别硬编码是否允许浪漫关系的分支。未来更多
性别身份和表达方式属于 Actor / WorldPack 的真实需求，应在获得压力测试后
扩展，而不是通过新增平行关系系统解决。

## 49.2 所有亲密关系参与者必须是成年人

任何情侣、亲密关系或联合成长行动都只允许明确为成年人的角色参与。年龄
未知、未成年或年龄状态矛盾时，关系和亲密联合行动必须不可用。

这是未来系统不可妥协的 invariant，不是 Narrator 可以用暧昧措辞绕开的判断。
成人资格必须来自权威 Actor 状态或 WorldPack 事实，不能由玩家、LLM 或自由
文本临时推断。

## 49.3 关系是双方状态，不是玩家单方面拥有的奖励

情侣关系必须表示两个独立 Actor 之间的真实关系，而不是：
...
关系成立至少需要双方身份有效、双方符合各自偏好、双方已经形成足够的关系
基础、双方明确同意，并且关系尚未终止。玩家提出关系不等于关系成立；NPC
不能因为玩家是主角而失去自主意愿。

关系也不能只由玩家一侧的数值或旗标代表。未来如果一方拒绝、撤回同意或
终止关系，该变化必须成为权威世界事实，并影响后续可用行动。

## 49.4 同意必须明确、持续并由权威状态记录

没有双方明确同意，不能形成 active romantic bond。没有双方对当前联合行动
的有效同意，不能执行双修或其他亲密联合行动。

同意必须由合法 Action、确定性规则和权威 Event 确认。Narrator、LLM Player
或自由文本不得自行推断 NPC 已经同意。拒绝、撤回和关系终止必须被尊重，
不得通过改写叙事、补写对白或隐含概率绕过。

关系同意和一次具体联合行动的同意也不是同一个事实：拥有 active bond 不
自动等于同意所有未来行动。每次需要额外同意的行动，都必须在其合法性合同
中单独验证当前同意状态。

## 49.5 性取向和私人偏好属于角色事实与知识边界

角色可以拥有自己的 gender identity、romantic orientation、partner
preferences、current commitments 和 private feelings。这些是角色事实或
角色私有状态，不是 Engine 的宇宙常量，也不默认全部暴露给玩家。
...
Player Knowledge 仍然必须分离；关系内容不能成为新的全知信息通道。
...
数值可以作为内部关系事实的一部分，但关系还必须受到共同经历、时间投入、
行为一致性、承诺、信息、目标冲突、双方选择和机会成本影响。关系系统的
第一 vertical slice 可以很小，但不能把“刷数值”确立为长期设计方向。

## 49.7 双修是 Relationship-produced Capability

双修在架构上不是性别系统，也不是被动经验 Buff。它应当是由真实关系状态
产生的联合行动或联合 Capability。通用架构语言应使用：

```text
Relationship-produced Capability
Mutual Joint Action
```

第一 WorldPack 可以暂时使用 `DUAL_CULTIVATE` 作为候选名称，但这只是一个
题材名称，不是所有世界都必须使用“修炼”主题。

未来联合行动至少应受到有效双方关系、双方明确同意、合法地点或环境、双方
当前可行动状态，以及真实时间、体力、资源或机会成本约束。它必须改变策略
空间，例如共同获得某种成长、打开只有该组合能执行的行动、协同利用知识，
或形成单人无法成立的成长路径。

选择联合成长必须意味着放弃其他事情：双方在这段时间不能探索，可能错过
世界机会，消耗共同资源，暴露于风险，或形成新的承诺。它不能退化为：
...
两人已经相爱、NPC 已经同意、两人完成了双修，或关系因此升级，除非对应
变化已经由合法 Action、Event 和 Reducer 产生。
...
任何未来设计都应先回答：删除浪漫叙事以后，关系、同意、联合行动、成本和
后果是否仍然能从结构化状态与事件成立。如果不能，它只是文本效果，不是
可验证的世界机制。
